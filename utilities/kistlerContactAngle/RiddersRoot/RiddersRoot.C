...
Author
    ...
    Modified by Thomas C. Sykes, CDT in Fluid Dynamics, University of Leeds, UK.

Notes
    Taken from CFD-PC github code.
    (18-Jul-2018) Use Foam namespace individually (not set everywhere).
...
const Foam::label Foam::RiddersRoot::maxIter = 60;


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::RiddersRoot::RiddersRoot(const FuncBase& f , const scalar eps)
:
    f_(f),
    eps_(eps)
{}
...
Foam::scalar Foam::RiddersRoot::root
(
    const scalar x0,
    const scalar x1
) const
